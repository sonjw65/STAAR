...
def _log_model_info(cfg: BasicTSConfig, logger=None) -> None:
    """Log model configuration and summary after device setup."""

    model_class = cfg.get("model", None)
    model_config = cfg.get("model_config", None)
    if model_class is None or model_config is None:
        return

    model_cfg_dict = asdict(model_config) if is_dataclass(model_config) else dict(model_config)
    config_payload = {
        ...
        "lr_scheduler": cfg.lr_scheduler.__name__ if cfg.lr_scheduler is not None else None,
        "lr_scheduler_params": cfg.lr_scheduler_params,
        "model_config": model_cfg_dict,
    }
    config_text = json.dumps(config_payload, indent=4, ensure_ascii=True)
    if logger is not None:
        logger.info("Model Configuration\n%s", config_text)

    model = model_class(model_config)
    summary_fn = None
    torchinfo_spec = importlib.util.find_spec("torchinfo")
    if torchinfo_spec is not None:
        summary_fn = importlib.import_module("torchinfo").summary

    if summary_fn is not None:
        summary_stats = summary_fn(
            model,
            input_size=[
                (cfg.train_batch_size, model_config.input_len, model_config.num_nodes),
                (cfg.train_batch_size, model_config.input_len, 2),
            ],
        ...
        fallback_msg = "torchinfo is not installed. Install it to print a detailed layer table: pip install torchinfo"
        if logger is not None:
            logger.info(fallback_msg)
            logger.info("Printing model module tree instead.")
            logger.info("Model module tree\n%s", model)
...
def training_func(cfg: BasicTSConfig):
    ...
    if get_rank() == 0:
        _log_model_info(cfg, logger=runner.logger)
    # train
    try:
        runner.train()
