...
DEFAULT_DATA_ROOT_PATH = "datasets"


def get_data_root_path() -> str:
    """Return data root path, allowing env override."""

    return os.environ.get("BASICTS_DATA_ROOT", DEFAULT_DATA_ROOT_PATH)


def get_data_file_path(dataset_name: str, file_name: str = "") -> str:
    """Return dataset directory or a file path under it."""

    dataset_dir = os.path.join(get_data_root_path(), dataset_name)
    if not file_name:
        return dataset_dir
...
def load_meta_description(dataset_name: str) -> str:
    ...
    # read json file: <dataset_root>/<dataset_name>/meta.json
    with open(get_data_file_path(dataset_name, 'meta.json'), 'r') as f:
        desc = json.load(f)
    return desc
...
def load_adj(dataset_name: str, adj_type: str):
    ...
    file_path = get_data_file_path(dataset_name, 'adj_mx.pkl')
