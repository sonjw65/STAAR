...
class BasicTSRunner:
    ...
    def __init__(self, cfg: "BasicTSConfig") -> None:
        ...
        if isinstance(cfg.loss, nn.Module):
            ...
            self.loss = types.FunctionType(
                ...
                _loss.__globals__,
                name="loss",
                argdefs=_loss.__defaults__,
                closure=_loss.__closure__,
            )
            self.loss.__kwdefaults__ = getattr(_loss, "__kwdefaults__", None)
            self.loss.__annotations__ = getattr(_loss, "__annotations__", {})
    ...
    def _test(self, train_step: Optional[int] = None, train_epoch: Optional[int] = None, \
              mode: BasicTSMode = BasicTSMode.TEST) -> None:
        """
    ...
    def _eval_loop(self, mode: BasicTSMode):
        ...
        for step, data in enumerate(data_iter):
            ...
            if mode == BasicTSMode.EVAL and getattr(self.cfg, "save_results", False):
                self._save_results(step, forward_return)
            # evaluation on specific timesteps
            if self.eval_horizons is not None and mode in [BasicTSMode.TEST, BasicTSMode.EVAL]:
